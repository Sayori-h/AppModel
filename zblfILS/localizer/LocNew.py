...
# 读取 CSV 文件
data = pd.read_csv('../../doc/loc.CSV', encoding='ISO-8859-1')
...
# 在 x = 11900 处添加一条参考线
# plt.axvline(x=11750, color='green', linestyle='--', linewidth=3, alpha=0.5, label='x=11750')

# 设置散点图的显示范围
plt.xlim(X.min(), X.max())  # 设置 X 轴显示范围为数据最小值到最大值
plt.ylim(Y.min(), Y.max())  # 设置 Y 轴显示范围为数据最小值到最大值
...
# 设置散点图的显示范围
plt.xlim(X.min(), X.max())  # 设置 X 轴显示范围为数据最小值到最大值
plt.ylim(residuals.min(), residuals.max())  # 设置 Y 轴显示范围为数据最小值到最大值
plt.xlabel('dis/m', fontsize=14)
