...
extra_Y = original_slope * extra_X + original_intercept + np.random.uniform(-0.03, 0.03, size=(extra_points, 1))
...
extra_Y_2 = np.random.uniform(low=-0.06, high=0.08, size=(extra_points_2, 1))

# 将新生成的数据点合并到原始数据中
X_new = np.concatenate((extra_X_2, X))
Y_new = np.concatenate((extra_Y_2, Y))
...
x_line_2 = np.linspace(-2.0, 2.0, 100).reshape(-1, 1)
y_line_2 = model.predict(x_line_2)
plt.plot(x_line_2, y_line_2, 'r-', linewidth=3, label='线性回归拟合曲线')

plt.ylabel('DDM', fontsize=21)
plt.xlabel('DOT', fontsize=21)
plt.legend(loc='upper right', fontsize=21)  # 将图例放置在右上角
# 设置坐标轴刻度字体大小
